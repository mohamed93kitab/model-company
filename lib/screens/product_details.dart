...
import 'package:model_company/custom/text_styles.dart';
import 'package:model_company/screens/cart.dart';
import 'package:model_company/screens/common_webview_screen.dart';
import 'package:model_company/screens/login.dart';
import 'package:model_company/screens/product_reviews.dart';
import 'package:model_company/screens/seller_details.dart';
import 'package:model_company/ui_elements/list_product_card.dart';
import 'package:model_company/ui_elements/mini_product_card.dart';
...
import 'package:model_company/repositories/product_repository.dart';
import 'package:model_company/repositories/wishlist_repository.dart';
...
import 'dart:async';
import 'package:model_company/screens/video_description_screen.dart';
...
class _ProductDetailsState extends State<ProductDetails> with TickerProviderStateMixin{
  ...
  setProductDetailValues() {
    ...
    if (_productDetails != null) {
      ...
      _productDetails.wholesale.forEach((wholesale) {
          _wholesaleList.add(wholesale);
      });
  ...
  addToCart({mode, context = null, snackbar = null}) async {
    ...
    var cartAddResponse = await CartRepository()
        .getCartAddResponse(widget.id, _variant, user_id.$, _quantity, _singlePrice.toDouble());
    ...
    if (cartAddResponse.result == false) {
      ...
      if (mode == "add_to_cart") {
       ...
       //   ScaffoldMessenger.of(context).showSnackBar(snackbar);
          ToastComponent.showPopup(AppLocalizations.of(context)
              .product_details_screen_snackbar_added_to_cart,  gravity: Toast.center,);
        }
        reset();
        fetchAll();
  ...
  buildQuantityUpButton() => Container(
        ...
        child: IconButton(
            ...
            onPressed: () {
              if (_quantity < _stock) {
                _quantity++;
                if(_wholesaleList.length > 0) {
                  for(var x = 0; x< _wholesaleList.length; x++) {
                    if(_wholesaleList[x].min_qty <= _quantity && _wholesaleList[x].max_qty >= _quantity) {
                      setState(() {
                        _singlePrice = int.parse(_wholesaleList[x].price);
                      });
                    }
                  }
                }
